...
Contact and estimate forms POST to a PHP handler on the same domain (`/api/submit.php`).
...
```
~/private/peninsula-pavers-mail.php
```
...
| `site_url`         | Optional — used in email templates (defaults to peninsulapavers.com) |
| `site_phone`       | Optional — display phone in email footer                             |
| `site_phone_href`  | Optional — tel: link for phone (`+12314218480`)                      |
...
Each form sends a hidden `form_type` field. Built-in types: `footer`, `drawer`, `contact`.
...
'forms' => [
    'footer' => [
        ...
        'subject' => 'Free estimate request — Example Co',
        'autoreply_subject' => 'We received your request — Example Co',
        'send_autoreply' => true,
    ],
    'contact' => [
        'subject' => 'New enquiry — Example Co',
        // send_autoreply defaults to false
    ],
],
```
...
| `{{submitted_at}}`    | Timestamp (America/Detroit)           |
...
| ----------- | ---------------------- | ---------------------------------------------- |
| `footer`    | `Footer.astro`         | `notification-footer.html`                     |
| `drawer`    | `EstimateDrawer.astro` | `notification-drawer.html` (includes services) |
...
1. Create `~/private/peninsula-pavers-mail.php` on the server (rename in `lib/mailer.php`
   for other client projects).
