...
{
    $paths = [
        dirname(__DIR__, 3) . '/private/peninsula-pavers-mail.php',
...
function allowedFormTypes(array $config): array
{
    ...
    return array_values(array_unique(array_merge(['footer', 'drawer', 'contact'], $configured)));
}
...
function getFormMailSettings(array $config, string $formType): array
{
    $defaults = [
        'footer' => [
            'source_label' => 'Footer estimate form',
            'subject' => 'Free estimate request — Peninsula Pavers',
            'autoreply_subject' => 'We received your request — Peninsula Pavers',
            'send_autoreply' => false,
        ],
        'drawer' => [
            'source_label' => 'Free estimate drawer',
            'subject' => 'Free estimate request — Peninsula Pavers',
            'autoreply_subject' => 'We received your request — Peninsula Pavers',
            'send_autoreply' => false,
        ],
...
function checkRateLimit(string $ip, array $config): bool
{
    ...
    $dir = sys_get_temp_dir() . '/peninsula-pavers-rate-limit';
