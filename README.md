...
| `site.ts` → `formEndpoint`                   | Form handler path (`/api/submit.php`)                    | optional   |
| `site.ts` → `recaptchaSiteKey`               | Google reCAPTCHA v3 site key                             | optional   |
...
| `npm run php:dev` | Build + PHPMailer + serve `dist/` with PHP on :8080      |
...
| `docs/FORMS-AND-EMAIL.md`     | Form handler, mail config, templates, local PHP testing |
